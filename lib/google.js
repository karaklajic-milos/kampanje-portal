...
// Autentikacija: OAuth refresh token (+ opciono login-customer-id za MCC nalog).
// Google je 9. septembra 2026. ukinuo developer tokene: nivo pristupa API-ju sada je vezan za Google Cloud projekat
// koji je vlasnik OAuth klijenta (stranica Google Ads API Overview). Developer token je opcion i ignoriše se.
...
function errorMessage(json, status) {
  ...
  let msg = e && e.message ? e.message : 'HTTP ' + status;
  const raw = JSON.stringify(json || '');
  if (/CLOUD_PROJECT_NOT_APPROVED_FOR_PRODUCTION|ACTION_NOT_PERMITTED/.test(raw)) {
    msg += ' (u Google Cloud-u otvorite Google Ads API Overview za projekat koji je vlasnik OAuth klijenta i zatražite Explorer pristup)';
  }
  return msg;
}
...
FROM campaign WHERE segments.date BETWEEN '${since}' AND '${until}'`;
  ...
  if (process.env.GOOGLE_ADS_DEVELOPER_TOKEN) headers['developer-token'] = process.env.GOOGLE_ADS_DEVELOPER_TOKEN; // opciono
