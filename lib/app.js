...
// Front-end fajlovi se učitavaju jednom, sa doslovnim putanjama, da ih Vercel uključi u paket funkcije.
const readAsset = (fn) => { try { return fn(); } catch (e) { return null; } };
const INDEX_HTML = readAsset(() => fs.readFileSync(path.join(__dirname, '..', 'public', 'index.html')));
const APP_JS = readAsset(() => fs.readFileSync(path.join(__dirname, '..', 'public', 'app.js')));
const STYLE_CSS = readAsset(() => fs.readFileSync(path.join(__dirname, '..', 'public', 'style.css')));

function cspHeader() {
  const ancestors = (process.env.FRAME_ANCESTORS || "'none'").split(/\s+/).filter(Boolean).join(' ');
  return "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; " +
    "font-src https://fonts.gstatic.com; img-src 'self' data:; connect-src 'self'; frame-ancestors " + ancestors +
...
function createApp() {
  ...
  app.use(helmet({ frameguard: false, contentSecurityPolicy: false })); // ugrađivanje kontroliše CSP frame-ancestors
  ...
  // Front-end servira sama aplikacija (isto lokalno i na Vercelu).
  const serve = (buf, type) => (req, res) => {
    if (!buf) return res.status(500).type('text/plain').send('Front-end fajlovi nisu pronađeni. Proveri da li je folder public otpremljen na GitHub.');
    res.set('Content-Type', type);
    res.set('Cache-Control', 'no-cache');
    res.send(buf);
  };
  app.get(['/', '/index.html'], serve(INDEX_HTML, 'text/html; charset=utf-8'));
  app.get('/app.js', serve(APP_JS, 'application/javascript; charset=utf-8'));
  app.get('/style.css', serve(STYLE_CSS, 'text/css; charset=utf-8'));
